...
const GenerateOAuthPackageArgsSchema = z.object({
  query: z.string().describe('Query describing the OAuth provider (e.g., "Google OAuth")'),
  url: z.string()
...
const OAuthPackageMetadataSchema = z.object({
  ...
  description: z.string().describe('A clear description of what this OAuth package does'),
 ...
 */
async function createInitialPackage(metadata: OAuthPackageMetadata): Promise<string> {
  ...
  fs.writeFileSync(path.join(srcDir, 'index.ts'), `// OAuth package content will be generated by AI`);
...
const WriteToFileSchema = z.object({
  sdkImplementation: z
    ...
    .describe(
      'The complete OAuth SDK implementation with separate files for main SDK, types, schemas, and exports',
    ),
  extraInfo: z
    .array(z.string())
    .describe(
...
if (require.main === module) {
  ...
  if (!queryArg) {
    ...
    console.log('Usage: node generate-oauth-package.js "OAuth Provider/Query" "URL to scrape"');
    process.exit(1);
  }
  ...
  generateOAuthPackage({ query: queryArg, url: urlArg })
    .then(result => {
      if (result) {
        console.log(`✅ OAuth package generation complete for ${result.packageName}`);
