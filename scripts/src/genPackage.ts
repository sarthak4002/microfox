...
const SDKMetadataSchema = z.object({
  ...
  authSdk: z
    .string()
    .optional()
    .describe('The OAuth SDK to use (e.g., "@microfox/google-oauth"). Required when authType is "oauth2"'),
});
 ...
 */
async function summarizeContent(content: string, query: string): Promise<string> {
...
export async function generateSDK(
  ...
  try {
    ...
    const docsSummary = await summarizeContent(combinedContent, validatedArgs.query);
