...
import { createSlackSDK, BlockText, SlackMessage, UpdateMessage, FileUpload } from '../slack-sdk';
import { z } from 'zod';
...
vi.mock('@microfox/rest-sdk', () => {
    ...
    const mockJson = vi.fn().mockRejectedValue(new Error('API Error: Invalid token'));
...
describe('Slack SDK Validation and Error Handling', () => {
    ...
    describe('SDK Initialization Validation', () => {
        it('should throw error with empty token', () => {
            ...
            expect(() => createSlackSDK({ botToken: '' })).toThrow(/Bot token cannot be empty/);
        });
    ...
    describe('sendMessage Validation', () => {
        it('should reject messages missing required channel field', async () => {
            ...
            await expect(slackSDK.sendMessage(invalidMessage))
                .rejects
                .toThrow(/required/i);
        });
        ...
        it('should handle API errors gracefully with custom error message', async () => {
            ...
            await expect(slackSDK.sendMessage(mockMessage))
                .rejects
                .toThrow('Failed to send Slack message: API Error: Invalid token');
        });
    });
    ...
    describe('updateMessage Validation', () => {
        it('should reject updates missing required channel field', async () => {
            ...
            await expect(slackSDK.updateMessage(invalidUpdate))
                .rejects
                .toThrow(/required/i);
        });
        ...
        it('should handle API errors gracefully with custom error message', async () => {
            await expect(slackSDK.updateMessage(mockUpdateMessage))
                .rejects
                .toThrow('Failed to update Slack message: API Error: Invalid token');
        });
    });

    describe('uploadFile Validation', () => {
        it('should handle API errors gracefully with custom error message', async () => {
            await expect(slackSDK.uploadFile(mockFileUpload))
                .rejects
                .toThrow('Failed to upload file to Slack: API Error: Invalid token');
        });
    ...
    describe('Block Kit Schema Validation', () => {
        ...
        it('should validate button parameters', () => {
            // Valid button
            expect(() => slackSDK.blocks.button('Click me', 'action_id')).not.toThrow();

            // Valid button with style
            expect(() => slackSDK.blocks.button('Danger', 'action_id', {
                style: 'danger'
            })).not.toThrow();
