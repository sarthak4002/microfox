...
const BlockTextSchema = z.object({
    type: z.enum(['plain_text', 'mrkdwn'])
        .describe('The formatting type to use for this text object. Use plain_text for raw text, or mrkdwn for markdown-formatted text.'),
    text: z.string()
        .describe('The actual text content to display.'),
    emoji: z.boolean().optional()
        .describe('When type is plain_text, indicates whether emojis in this text should be escaped into the colon emoji format.'),
    verbatim: z.boolean().optional()
        .describe('When type is mrkdwn, indicates whether to escape special characters like & < and >.'),
}).strict();
...
const BlockImageSchema = z.object({
    ...
    alt_text: z.string()
        .describe('A plain-text summary of the image for accessibility purposes.'),
    title: BlockTextSchema.optional()
        .describe('An optional title that appears above the image.'),
...
const BlockButtonSchema = z.object({
    type: z.literal('button')
        .describe('The type of block element. Must be "button" for button elements.'),
    text: BlockTextSchema
        .describe('The text that appears on the button.'),
    action_id: z.string()
        .describe('A unique identifier for the action triggered when this button is clicked.'),
    url: z.string().url().optional()
        .describe('A URL to redirect to when this button is clicked.'),
    value: z.string().optional()
        .describe('A custom value to include with the action payload when this button is clicked.'),
    style: z.enum(['primary', 'danger']).optional()
        .describe('The visual style of the button. Primary is green, danger is red.'),
    confirm: z.any().optional()
        .describe('A confirmation dialog that appears before the action is executed.'),
}).strict();

const BlockSectionSchema = z.object({
    type: z.literal('section')
        .describe('The type of block. Must be "section" for section blocks.'),
    text: BlockTextSchema.optional()
        .describe('The primary text content of the section.'),
    block_id: z.string().optional()
        .describe('A unique identifier for this block.'),
    fields: z.array(BlockTextSchema).optional()
        .describe('An array of text objects to display in a columnar format.'),
    accessory: z.union([BlockButtonSchema, BlockImageSchema]).optional()
        .describe('An optional interactive or visual element to accompany the section.'),
}).strict();

const BlockDividerSchema = z.object({
...
const BlockSchema = z.union([
    ...
    BlockImageSchema,
    BlockDividerSchema,
]).describe('A Block Kit block that can be used to compose rich message layouts.');

// Legacy attachment schemas
const AttachmentFieldSchema = z.object({
    title: z.string()
        .describe('The title of the field.'),
    value: z.string()
        .describe('The value of the field.'),
    short: z.boolean().optional()
        .describe('Indicates whether this field should be displayed as a short field (side-by-side with another field).'),
}).strict();

const AttachmentSchema = z.object({
    fallback: z.string().optional()
        .describe('A plain text summary of the attachment used in notifications.'),
    color: z.string().optional()
        .describe('The color to use for the attachment. Can be hex (#FF0000) or predefined colors like "good", "warning", "danger".'),
    ...
    author_icon: z.string().url().optional()
        .describe('A URL that displays a small image to the left of the author_name text.'),
    title: z.string().optional()
        .describe('The title of the attachment.'),
    ...
    fields: z.array(AttachmentFieldSchema).optional()
        .describe('An array of field objects that get displayed in a table-like format.'),
    image_url: z.string().url().optional()
        .describe('A URL to an image file that will be displayed inside the attachment.'),
    thumb_url: z.string().url().optional()
        .describe('A URL to an image file that will be displayed as a thumbnail on the right side.'),
    footer: z.string().optional()
        .describe('Text that appears in the footer section.'),
    footer_icon: z.string().url().optional()
        .describe('A URL to an image file that will be displayed to the left of the footer text.'),
...
const SlackMessageSchema = z.object({
    channel: z.string()
        .describe('The channel ID or name where the message should be posted (e.g., "#general" or "C1234567890").'),
    text: z.string()
        .describe('The text content of the message. When using blocks, this becomes the fallback text for notifications.'),
    blocks: z.array(BlockSchema).optional()
        .describe('An array of Block Kit blocks that define the rich layout of the message.'),
    attachments: z.array(AttachmentSchema).optional()
        .describe('Legacy message attachments for backward compatibility.'),
    thread_ts: z.string().optional()
        .describe('The timestamp of a parent message to reply to, creating a thread.'),
    reply_broadcast: z.boolean().optional()
        .describe('When replying to a thread, this broadcasts the message to the channel as well.'),
    mrkdwn: z.boolean().optional()
        .describe('Determines whether markdown formatting is enabled for this message.'),
    parse: z.enum(['none', 'full']).optional()
        .describe('Change how messages are treated: "none" disables linkification, "full" enables it.'),
    ...
    icon_emoji: z.string().optional()
        .describe('Custom emoji to use as the bot icon (e.g., ":chart_with_upwards_trend:").'),
...
const SlackMessageResponseSchema = z.object({
    ...
    ts: z.string().optional()
        .describe('The timestamp of the posted message, used as a unique identifier.'),
    message: z.union([MessageResponseSchema, z.record(z.unknown())]).optional()
        .describe('Details about the posted message.'),
    error: z.string().optional()
        .describe('Error message if the request failed.'),
    warning: z.string().optional()
        .describe('Warning message from Slack API.'),
...
const FileUploadSchema = z.object({
    channels: z.string().optional()
        .describe('Comma-separated list of channel IDs where the file should be shared.'),
    ...
    title: z.string().optional()
        .describe('Title of the file being uploaded.'),
}).passthrough();
...
const FileUploadResponseSchema = z.object({
    ...
    file: z.object({
        id: z.string()
            .describe('The unique identifier of the uploaded file.'),
        created: z.number().optional()
            .describe('Timestamp when the file was created.'),
        timestamp: z.number().optional()
            .describe('Timestamp when the file was uploaded.'),
        name: z.string()
            .describe('The name of the uploaded file.'),
        title: z.string()
            .describe('The title of the uploaded file.'),
        mimetype: z.string()
            .describe('The MIME type of the file.'),
        filetype: z.string()
            .describe('The Slack filetype identifier.'),
        ...
        mode: z.string().optional()
            .describe('File mode/permissions.'),
...
// Helper functions
const createBlockText = (text: string, type: 'plain_text' | 'mrkdwn' = 'plain_text'): BlockText => ({
    type,
    text,
});

const createSection = (text: string | BlockText, fields?: BlockText[]): BlockSection => ({
    type: 'section',
...
const createButton = (text: string, actionId: string, options: Partial<Omit<BlockButton, 'type' | 'text' | 'action_id'>> = {}): BlockButton => ({
    type: 'button',
...
export const createSlackSDK = (config: SlackSDKConfig): SlackSDK => {
    ...
    return {
         ...
         */
        sendMessage: async (message: SlackMessage): Promise<SlackMessageResponse> => {
         ...
         */
        updateMessage: async (message: UpdateMessage): Promise<SlackMessageResponse> => {
