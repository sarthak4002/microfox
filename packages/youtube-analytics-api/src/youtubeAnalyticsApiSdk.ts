...
const response = await this.makeRequest<ReportResponse>('/reports', 'GET', {
  ...validatedParams,
  ids: params.ids ?? '',
...
const response = await this.makeRequest<GroupListResponse>('/groups', 'GET', {
...
await this.makeRequest<void>(`/groups/${id}`, 'DELETE', { onBehalfOfContentOwner: onBehalfOfContentOwner ?? '' });
}
...
const response = await this.makeRequest<GroupItemListResponse>('/groupItems', 'GET', {
...
await this.makeRequest<void>(`/groupItems/${id}`, 'DELETE', { onBehalfOfContentOwner: onBehalfOfContentOwner ?? '' });
}
}
