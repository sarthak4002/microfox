...
export const createGithubSdk = (config: GithubSdkConfig) => {
  ...
  const createRepositoryFromTemplate = async (params: RepoParams) => {
    return await restSdk.post(`/repos/${config.owner}/${config.repo}/generate`, {
  ...
  const createPullRequest = async (params: PRParams) => {
    return await restSdk.post(`/repos/${config.owner}/${config.repo}/pulls`, params).json();
  };

  const createCommentOnPr = async (prNumber: number, comment: string) => {
    return await restSdk.post(
  ...
  const getAllPullRequests = async () => {
    return await restSdk.get(`/repos/${config.owner}/${config.repo}/pulls`).json();
  };

  const getPrDetails = async (prNumber: number) => {
    return await restSdk.get(`/repos/${config.owner}/${config.repo}/pulls/${prNumber}`).json();
  };

  // Branch Management
  const getBranchSha = async (branch: string): Promise<string> => {
    const refData = await restSdk.get<{ object: { sha: string } }>(
  ...
  const createBranch = async (params: BranchParams) => {
    ...
    return await restSdk.post(`/repos/${config.owner}/${config.repo}/git/refs`, {
  ...
  const getBranches = async () => {
    return await restSdk.get(`/repos/${config.owner}/${config.repo}/branches`).json();
  };

  // File Management
  const getFileContent = async (path: string, branch: string = 'main') => {
    return await restSdk.get(
  ...
  const updateFile = async (params: UpdateFileParams) => {
    ...
    try {
      currentFileData = await getFileContent(params.path, params.branch) as any;
    } catch (error) {
  ...
  const getPrEvents = async (prNumber: number, per_page: number = 30, page: number = 1) => {
    const events = await restSdk.get<any[]>(
    ...
    const timeline = await restSdk.get<any[]>(
