...
export const UserSchema = z.object({
  ...
  created_at: z.string().optional().describe('The UTC datetime that the user account was created.'),
  protected: z.boolean().optional().describe('Indicates if this user has chosen to protect their Tweets.'),
  profile_image_url: z.string().optional().describe('URL to the profile image for this user.'),
  description: z.string().optional().describe('The user-defined UTF-8 string describing their account.'),
  location: z.string().optional().describe('The user-defined location for this account\'s profile.'),
  url: z.string().optional().describe('The URL specified in the user\'s profile.'),
  verified: z.boolean().optional().describe('Indicates if this user is verified.'),
  verified_type: z.string().optional().describe('The type of verification the user has.'),
  public_metrics: z.object({
    followers_count: z.number().optional().describe('Number of users who follow this user.'),
    following_count: z.number().optional().describe('Number of users this user follows.'),
    tweet_count: z.number().optional().describe('Number of Tweets posted by this user.'),
    listed_count: z.number().optional().describe('Number of lists that include this user.'),
  }).optional().describe('Public engagement metrics for the user.'),
}).passthrough();

// Tweet Schemas
export const TweetGeoSchema = z.object({
  place_id: z.string().describe('Place ID being attached to the Tweet for geo location.'),
});

export const TweetMediaSchema = z.object({
  media_ids: z.array(z.string()).describe('A list of Media IDs to be attached to a created Tweet.'),
  tagged_user_ids: z.array(z.string()).optional().describe('A list of User IDs to be tagged in the media.'),
});

export const TweetPollSchema = z.object({
  duration_minutes: z.number().min(5).max(10080).describe('Duration of the poll in minutes.'),
  options: z.array(z.string()).describe('The text options for the poll.'),
  reply_settings: z.enum(['following', 'mentionedUsers']).optional().describe('Reply settings.'),
});

export const TweetReplySchema = z.object({
  in_reply_to_tweet_id: z.string().describe('The ID of the Tweet being replied to.'),
  exclude_reply_user_ids: z.array(z.string()).optional().describe('User IDs to exclude from reply.'),
});
...
export const TweetCreateSchema = z.object({
  ...
  community_id: z.string().optional().describe('The unique identifier of the Community.'),
  direct_message_deep_link: z.string().optional().describe('Link to private Direct Message.'),
  for_super_followers_only: z.boolean().optional().default(false).describe('Super followers tweet.'),
  geo: TweetGeoSchema.optional().describe('Geo location.'),
  media: TweetMediaSchema.optional().describe('Media information.'),
  nullcast: z.boolean().optional().default(false).describe('Promoted-only Posts.'),
  poll: TweetPollSchema.optional().describe('Poll options.'),
  quote_tweet_id: z.string().optional().describe('ID of the Tweet being quoted.'),
  reply: TweetReplySchema.optional().describe('Reply information.'),
  reply_settings: z.enum(['following', 'mentionedUsers', 'subscribers']).optional(),
});
...
export const TweetResponseSchema = z.object({
  ...
  author_id: z.string().optional().describe('The user who posted this Tweet.'),
  edit_history_tweet_ids: z.array(z.string()).optional().describe('Previous tweet IDs.'),
}).passthrough();
...
export const MultipleUsersLookupResponseSchema = z.object({
...
export type MultipleUsersLookupResponse = z.infer<typeof MultipleUsersLookupResponseSchema>;
...
export interface UsersAPI {
  getByUsername: (username: string, options?: { expansions?: string[] }) => Promise<UserLookupResponse>;
  getByUsernames: (usernames: string[], options?: { expansions?: string[] }) => Promise<MultipleUsersLookupResponse>;
  getById: (id: string, options?: { expansions?: string[] }) => Promise<UserLookupResponse>;
  getByIds: (ids: string[], options?: { expansions?: string[] }) => Promise<MultipleUsersLookupResponse>;
  getMe: (options?: { expansions?: string[] }) => Promise<UserLookupResponse>;
}

export interface PostsAPI {
  create: (tweet: TweetCreate) => Promise<TweetResponse>;
  get: (id: string, options?: { expansions?: string[] }) => Promise<TweetLookupResponse>;
  getMultiple: (ids: string[], options?: { expansions?: string[] }) => Promise<TweetLookupResponse>;
...
export interface XSDK {
  ...
  users: UsersAPI;
  media: MediaAPI;
  generateOAuthHeader: (method: string, url: string, params?: Record<string, string>) => string;
}
...
export const createXSDK = (config: XSDKConfig): XSDK => {
          ...
          res.on('end', () => {
            ...
            try {
              ...
              if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
                ...
                reject(new XError(`Request failed with status ${res.statusCode}`, parsed));
              }
            } catch (error) {
  ...
  const tweets: PostsAPI = {
    create: async (tweet: TweetCreate) => {
      try {
        ...
        if (!response.data) throw new XError('API response missing data object');
  ...
  const users: UsersAPI = {
    getByUsername: async (username: string, options?: { expansions?: string[] }) => {
      try {
        const params: Record<string, string> = {};
        if (options?.expansions?.length) {
          params.expansions = options.expansions.join(',');
        }
        const response = await request('GET', `/2/users/by/username/${encodeURIComponent(username)}`, params);
    ...
    getByUsernames: async (usernames: string[], options?: { expansions?: string[] }) => {
      try {
            ...
            res.on('end', () => {
              ...
              if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
                try {
                  ...
                  const validated = MediaUploadResponseSchema.safeParse(parsedData);
                  if (validated.success) {
                    resolve(validated.data);
                  } else {
                    reject(new XError(`Invalid response format: ${validated.error.message}`));
                  }
                } catch (error) {
                  reject(new XError(`Failed to parse response: ${error}`));
                }
              } else {
                reject(new XError(`HTTP ${res.statusCode}: ${res.statusMessage} - ${data}`));
              }
            });
          });
