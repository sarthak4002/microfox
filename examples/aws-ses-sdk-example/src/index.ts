...
const createSESClient = (config: { accessKeyId: string; secretAccessKey: string; region: string }) => {
    return createSESSdk({
...
async function sesSDKExamples() {
    ...
    if (!senderEmail || !recipientEmail) {
        throw new Error('SENDER_EMAIL and RECIPIENT_EMAIL environment variables are required.');
    }
    ...
    try {
        ...
        const plainTextEmail = await sesSDK.sendEmail({
            ...
            bodyText: 'This is a plain text email sent using the AWS SES SDK. No HTML content included.',
      ...
      `,
        ...
        try {
            ...
            console.log('❌ Error: Email with both body formats was accepted (should not happen)');
        } catch (error) {
            console.log('✅ Validation correctly prevented sending email with both body formats');
