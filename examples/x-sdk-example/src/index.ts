...
const createXClient = () => {
  ...
  if (!apiKey || !apiSecret || !accessToken || !accessSecret) {
    throw new Error(
      'X API credentials are required (X_API_KEY, X_API_SECRET, X_ACCESS_TOKEN, X_ACCESS_SECRET)',
    );
  }
...
async function xSDKExamples() {
  try {
    ...
    const multipleUsers = await xSDK.users.getByUsernames([
      'elonmusk',
      'spacex',
    ]);
    ...
    if (fs.existsSync(testImagePath)) {
      ...
      if (mediaResponse.media_id_string) {
        const mediaTweet = await xSDK.tweets.create({
          text:
            'Testing media upload with the X SDK at ' +
      ...
      console.log(
        `Media upload skipped - create a file at ${testImagePath} to test this feature`,
      );
    }
...
async function errorHandlingExample() {
  ...
  try {
    ...
    const response = await xSDK.users.getByUsername(
      'thisuserprobablydoesnotexist12345678',
    );
